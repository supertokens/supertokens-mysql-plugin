...
public class TOTPQueries {
    ...
    private static int insertUser_Transaction(Start start, Connection con, String userId)
        ...
        String QUERY = "INSERT IGNORE INTO " + Config.getConfig(start).getTotpUsersTable()
                + " (user_id) VALUES (?)";
        // Note: "INSERT IGNORE" is used to avoid throwing an exception if the user
        // already exists. Not using "ON DUPLICATE KEY UPDATE" because there's only
        // one column in the table and it's the primary key. Plus, INSERT IGNORE is
        // more efficient.
