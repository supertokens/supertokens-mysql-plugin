...
public class WebAuthNQueries {
    ...
    public static void createUser_Transaction(Start start, Connection sqlCon, TenantIdentifier tenantIdentifier, String userId, String email,
        ...
        try {
            // app_id_to_user_id
            String insertAppIdToUserId = "INSERT INTO " + Config.getConfig(start).getAppIdToUserIdTable()
            ...
            String insertAllAuthRecipeUsers = "INSERT INTO " + Config.getConfig(start).getUsersTable()
                    +
    ...
    public static Collection<? extends LoginMethod> getUsersInfoUsingIdList_Transaction(Start start, Connection connection, Set<String> ids, AppIdentifier appIdentifier)
        ...
        if (ids.size() > 0) {
            ...
            String webauthnUsersTable = Config.getConfig(start).getWebAuthNUsersTable();
            String credentialTable = Config.getConfig(start).getWebAuthNCredentialsTable();
    ...
    public static AuthRecipeUserInfo getUserInfoByCredentialId_Transaction(Start start, Connection sqlCon, TenantIdentifier tenantIdentifier, String credentialId)
        ...
        String QUERY = "SELECT webauthn.user_id as user_id, webauthn.email as email, webauthn.time_joined as time_joined, " +
                "credentials.id as credential_id, email_verification.email as email_verified, user_id_mapping.external_user_id as external_user_id," +
                "all_users.tenant_id as tenant_id " +
                "FROM " + Config.getConfig(start).getWebAuthNUsersTable() + " as webauthn " +
                "JOIN " + Config.getConfig(start).getUsersTable() + " as all_users ON webauthn.app_id = all_users.app_id AND webauthn.user_id = all_users.user_id " +
                "LEFT JOIN " + Config.getConfig(start).getWebAuthNCredentialsTable() + " as credentials ON webauthn.user_id = credentials.user_id " +
                "LEFT JOIN " + Config.getConfig(start).getUserIdMappingTable() + " as user_id_mapping ON webauthn.user_id = user_id_mapping.supertokens_user_id " +
                "LEFT JOIN " + Config.getConfig(start).getEmailVerificationTable() + " as email_verification ON webauthn.app_id = email_verification.app_id AND user_id_mapping.external_user_id = email_verification.user_id OR user_id_mapping.supertokens_user_id = email_verification.user_id " +
